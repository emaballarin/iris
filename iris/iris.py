...
from .host import LocalPath
from .host import RemotePath
from .host import run

logging.basicConfig()

parser = argparse.ArgumentParser(
    description="iris is a command line tool to sync folders between local and remote"
)
...
class IrisConfig:
    ...
    def static_dict():
        return {
            k.name: k.default if k.default is not MISSING else ""
            for k in fields(IrisConfig)
        }
...
class PrettyConsole(Console):
    ...
    def callback_write(self, from_host, path, to_host, rev=False):
        return partial(
            self.log_write, from_host=from_host, path=path, to_host=to_host, rev=rev
        )

    def callback_progress(self, name):
        task_id = self.progress.add_task(
            f"[bold blue]Reading: {name}", total=1, visible=False
        )
        return partial(self._callback_progress, id=task_id, name=name)

    def _callback_progress(self, adv, cbpid, name):
        if adv is True:  # Use this to trigger completion
            self.progress.remove_task(cbpid)
            return
        if adv is False:
            self.progress.reset(
                cbpid, description=f"[bold blue]Writing: {name}", visible=False
            )
            return
        # noinspection PyProtectedMember
        self.progress._tasks[cbpid].visible = True
        self.progress.update(cbpid, advance=adv)
    ...
    def log_write(self, merged, from_host, path, to_host, change, rev):
        ...
        subs = (
            (from_host, change, to_host, path)
            if rev
            else (from_host, path, change, to_host)
        )
...
def init_config():
    ...
    parser.add_argument(
        "--config", type=str, help="yaml config file path", default="iris_conf.yaml"
    )
    args = parser.parse_args()

    if os.path.isfile(args.config):
        console.print(
            f"[bold red] The configuration file [bold green]{args.config}[/bold green] already exists, exiting."
        )
        sys.exit(0)
...
def main():
    ...
    def file_path(string):
        if os.path.isfile(string):
            ...
            console.print(
                f"[green]{string}[/green] [red] - Config file not found[/red]"
            )
            sys.exit()
    ...
    parser.add_argument(
        "--dry", action="store_true", help="Fake run with no file writing"
    )
    ...
    with Live(progress_group, console=console):
        ...
        if not from_path.check_connection():
            ...
            sys.exit()
        console.print(
            f"Connection to [green]{from_path.host}:{from_path.path}[/green] established"
        )
        if not to_path.check_connection():
            ...
            sys.exit()
        console.print(
            f"Connection to [green]{to_path.host}:{to_path.path}[/green] established"
        )
        ...
        console.print(
            f"[green]{len(from_files)}[/green] Files received from [green]{from_path.host}"
        )

        if config.mirror:
            to_files = to_path.all_files()
            console.print(
                f"[green]{len(to_files)}[/green] Files received from [green]{to_path.host}"
            )

        # For each file do a merge on both sides.
        status.update(
            status=f"[bold blue] Merging files from "
        ...
        tasks = [
            from_path.write(
                f,
                to_path,
                console.callback_write(from_path.host, f.short_path, to_path.host),
        ...
        if config.mirror:
            ...
            tasks = [
                to_path.write(
                    f,
                    from_path,
                    console.callback_write(
                        from_path.host, f.short_path, to_path.host, True
                    ),
                    console.callback_progress(f.short_path),
                )
                for f in to_files
        ...
        console.print(
            f"[bold blue]Initial sync completed in {(time.time() - t0):.2f} seconds"
        )
        ...
        status.update(
            status="[bold blue]Listening for changes on local and remote path"
        )
                ...
                for r in from_req:
                    req.append(
                        from_path.write(
                            ...
                            to_path,
                            console.callback_write(
                                from_path.host, r.short_path, to_path.host
                            ),
                ...
                for r in to_req:
                    req.append(
                        to_path.write(
                            ...
                            from_path,
                            console.callback_write(
                                from_path.host, r.short_path, to_path.host, True
                            ),
                            console.callback_progress(r.short_path),
                        )
                    )
